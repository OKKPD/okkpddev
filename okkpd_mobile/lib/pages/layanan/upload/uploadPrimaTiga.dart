...
class _UploadPrimaTigaState extends State<UploadPrimaTiga> {
  ...
  _uploadGambar(MediaModel media) async{
    final result = await Navigator.push(
      context,
      MaterialPageRoute(builder: (context) => TambahDokumenScreen(media.kodeDokumen)),
    );
...
//                ),
                Container(
                  child: Column(
                    ...
                    children: <Widget>[
                      Text(media.namaDokumen, style: TextStyle(fontWeight: FontWeight.bold, fontSize: 14),),
                      SizedBox(height: 16.0),
                      Expanded(
                        flex: 0,
                        child: _buildMedia(media.kodeDokumen)
                      ),
                      RaisedButton(child: Text("Tambah Dokumen"),onPressed: (){
                        _uploadGambar(media);
                      },)
